                              ...
                              std::vector<double>& u, std::vector<double>& v)
{
    ...
    for (int pass = 0; pass < nranks; ++pass) {
        // exchange
        const int dst = (rank + pass + nranks) % nranks;
        const int src = (rank - pass + nranks) % nranks;
        const int tag = 42;
        MPI_Sendrecv(x.data(), n, MPI_DOUBLE, dst, tag, xremote.data(), n,
                     MPI_DOUBLE, src, tag, comm, MPI_STATUS_IGNORE);
        MPI_Sendrecv(y.data(), n, MPI_DOUBLE, dst, tag, yremote.data(), n,
                     MPI_DOUBLE, src, tag, comm, MPI_STATUS_IGNORE);
        MPI_Sendrecv(gamma.data(), n, MPI_DOUBLE, dst, tag, gammaremote.data(),
                     n, MPI_DOUBLE, src, tag, comm, MPI_STATUS_IGNORE);
