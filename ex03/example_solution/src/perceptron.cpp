...
Perceptron::Perceptron(int in_dims, int max_batch)
  : in_dims_(in_dims),
    max_batch_(max_batch),
    weights_(new scalar[in_dims * out_dims_]),
    grad_(new scalar[in_dims * out_dims_]),
    outputs_(new scalar[max_batch * out_dims_])
{
    ...
    std::fill(weights_, weights_ + in_dims * out_dims_ , 1.0 / in_dims_);
    std::fill(grad_, grad_ + in_dims * out_dims_ , 0.0);
