...
public:
     ...
     */
    Perceptron(int in_dims, int out_dims, int max_batch);
...
protected:
     ...
     * @param out_dims_ Number of weight vectors
     * @param max_batch_ Upper limit for batch size which can be
     *                   passed to forward, update_weights or get_spectrum
     */
    const int in_dims_, out_dims_, max_batch_;
...
template<typename Func>
int Perceptron::set_weights(Func&& gen) {
